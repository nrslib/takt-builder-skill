...
エージェントが出力するレポートの構造定義。ワークフローYAMLの `report.format` フィールドで使用する。

| 項目 | 内容 |
|------|------|
| 目的 | エージェント出力の構造を統一し、後続ステップで機械的に参照可能にする |
| 配置 | Phase 2 メッセージの `## Instructions` 内（`{{outputContract}}`） |
| 対象 | 1つのレポートファイル |
| 判断基準 | 「この出力は後続ステップが `{report:filename}` で参照するか？」→ YES |
...
| 読者 | エージェント（実行時） | 後続ステップのエージェント（参照時） |
...
目的: 問題がない場合に冗長な出力を抑制する。後続ステップの入力コストを下げる。
...
3. **ワークフロー固有のルーティング**: 「REJECT の場合は fix ステップへ」等
...
番号プレフィックスはワークフロー構造に依存するため、共有ファイルでは使用しない。番号はワークフローYAMLの `report.name` フィールドで付ける。
